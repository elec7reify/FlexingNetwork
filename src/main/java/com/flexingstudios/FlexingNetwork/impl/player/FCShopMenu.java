...
public class FCShopMenu extends InvMenuImpl {
    ...
    public FCShopMenu(NetworkPlayer player) {
        super(Bukkit.createInventory(null, 54, "Магазин FlexCoin"));
        ...
        GLASS_PANE_BLACK_SLOTS.forEach(slot -> getInventory().setItem(slot, GLASS_PANE_BLACK));
        ...
        GLASS_PANE_BLUE_SLOTS.forEach(slot -> getInventory().setItem(slot, GLASS_PANE_BLUE));
        ...
        getInventory().setItem(11, Items.name(Material.CHEST, "&e&lТитулы", "&fТитулы &f- интересные приписки", "&fКоторые отображаются в чате и табе сразу после ника", "  &fНапример:",
                player.getRank().getDisplayName() + " &7" + player.getName() + " &5&lГачист", "", "&fВ магазине вы найдёте много разных титулов", "&fКоторые обязательно вам понравятся!", "", "&aНажмите, чтобы перейти."));

        getInventory().setItem(14, Items.name(Material.CHEST, "&b&lСообщения при входе", "&fСпециальные сообщения, которые приходят всем", "&fКогда вы зайдёте на сервер", "  &fНапример:",
                "&8&l[&c&l+&8&l] " + player.getRank().getDisplayName() + " &7" + player.getName() + " &aзалетел на тусу", "", "&aНажмите, чтобы перейти."));

        getInventory().setItem(39, Items.name(Material.CHEST, "&d&lГаджеты и плюшки &c&l(Survival only)", "&fМножество разнообразных плюшек", "&fОт крутых шапок до красивых баннеров",
                "&fИдеально подойдут, чтобы повеселиться с друзьями", "", "&aНажмите, чтобы перейти."));

        getInventory().setItem(42, Items.name(Material.CHEST, "&6&lСледы от стрел &c&l(Minigames only)", "&fРазличные следы при выстрелах из лука", "&fКрасиво, просто, и со вкусом", "", "&aНажмите, чтобы перейти."));
    }
    ...
    public void onClick(ItemStack item, NetworkPlayer player, int slot, ClickType clicktype) {
        switch (slot) {
            ...
            case 11:
                break;
            case 14:
                Invs.forceOpen(player.getBukkitPlayer(), new MsgJoinMenu(player).getInventory());
                break;
            case 39:
                break;
            case 42:
                Invs.forceOpen(player.getBukkitPlayer(), new ArrowTrailMenu(player, this).getInventory());
                break;
        }
    }
}
