...
public class MessageCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (command.getName().toLowerCase()) {
            case "msg":
                if (args.length < 2) {
                    Utils.msg(sender, "&cИспользование: /" + label + " &f<кому> <сообщение>");
                    return true;
                }
...
//                }
                ...
                if (player == null) {
                    Utils.msg(sender, "§cИгрок " + args[0] + " не найден");
                    break;
                }
            ...
            case "reply":
                if (args.length < 1) {
                    Utils.msg(sender, "&cИспользование: /" + label + " &f<сообщение");
                    return true;
                }
                senderInfo = (MysqlPlayer) FlexPlayer.get(sender.getName());
                message = args[0];
                ...
                if (senderInfo.lastWriter == null || (lastWriter = (MysqlPlayer) FlexPlayer.PLAYERS.get(senderInfo.lastWriter)) == null) {
                    Utils.msg(sender, "§cНекому ответить");
                    return true;
                }
    ...
    private void trySendPrivateMessage(MysqlPlayer sender, MysqlPlayer reciever, String message) {
        if (!sender.rank.has(Rank.ADMIN) && !FlexPlayer.get(reciever.getBukkitPlayer()).settings.get(1)) {
            Utils.msg(sender.player, reciever.getName() + " Отключил приватные сообщения");
            return;
        }

        if (!FlexPlayer.get(sender.getBukkitPlayer()).settings.get(1)) {
            Notifications.error(sender.player, sender.username, "Вы отключили личные сообщения");
            return;
        }

        if (sender.ignored.contains(reciever.getName())) {
             Notifications.error(sender.player, "FlexingWorld", "Игрок " + reciever.getName() + " у Вас в игноре.");
            return;
        }

        if (!sender.rank.has(Rank.ADMIN) && reciever.ignored.contains(sender.getName())) {
            Notifications.error(sender.player, reciever.getName(),"&cВы в игноре у этого игрока");
            return;
        }
        ...
        //reply
        TextComponent reply = new TextComponent(new ComponentBuilder(Utils.colored("&7[↩] &8⇢ "))
        ...
        TextComponent part = new TextComponent(Utils.colored("[&6" + sender.player.getDisplayName() + "&r -> &6Вы&r] "));
        BaseComponent[] components = new BaseComponent[]{ part, reply, componentMessage };

        // formatted player name
        TextComponent playerName = new TextComponent(new ComponentBuilder(Utils.colored("&6" + reciever.getName()))
        ...
        TextComponent part0 = new TextComponent(Utils.colored("[&6Вы&r -> "));
        TextComponent part1 = new TextComponent(playerName);
        TextComponent part2 = new TextComponent(Utils.colored("&r] "));
